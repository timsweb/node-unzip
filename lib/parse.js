...
function Parse(opts) {
  ...
  this._untilStream = new PullStream();
      ...
      self._untilStream.pull(vars.extraFieldLength, function (err, extraField) {
        ...
        if (vars.compressionMethod === 0) {
          ...
          if (fileSizeKnown) {
            ...
            self._untilStream.reconfigure({ pattern: descriptorSig});
            entry.on('finish', self._processDataDescriptor.bind(self, entry));
            self._untilStream.pipe(inflater).pipe(entry);
          }
        }
      });
    });
...
Parse.prototype._processDataDescriptor = function (entry) {
  var self = this;
  this._untilStream.reconfigure();
  this._untilStream.pull(16, function (err, data) {
    if (err) {
      return self.emit('error', err);
    }
